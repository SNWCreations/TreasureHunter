...
public class ChannelSession {
    ...
    public ChannelSession(SessionStorage storage, String channelId, long sessionAliveLength, User sender) {
        ...
        this.aliveLength = sessionAliveLength;
        this.board = new ChannelBoard();
    ...
    public void init() {
        ...
        }.runTaskLater(Main.getInstance(), aliveLength);
    ...
    private void upload() {
        ...
        User starter = JKook.getHttpAPI().getUser(starterId);
        String export = board.export();
        try (FileWriter fileWriter = new FileWriter(file)) {
            fileWriter.write(export);
        } catch (IOException e) {
            Main.getInstance().getLogger().error("Unable to write data.");
            starter.sendPrivateMessage(new TextComponent("抱歉！数据写入失败！我们会尝试将日志发送给您，您可以从日志中获取数据！"));
            ...
            starter.sendPrivateMessage(new FileComponent(logUrl, filename, -1, FileComponent.Type.FILE));
            return;
        }
        String s = JKook.getHttpAPI().uploadFile(file);
        starter.sendPrivateMessage(new FileComponent(s, filename, -1, FileComponent.Type.FILE));
    }

    private MultipleCardComponent renderStartCard() {
        long endTime = System.currentTimeMillis() + aliveLength;
